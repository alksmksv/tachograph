...
if uploaded_file:
  ...
  # --------------------------------------------------------------------------
  # СТРОГАЯ ИЗОЛИРОВАННАЯ ГРУППИРОВКА ПО МАШИНАМ (Appended Mini-Tables)
  # --------------------------------------------------------------------------
  # Каждая машина сортируется внутри себя по дате, а затем блоки «аппендятся» (склеиваются) друг за другом.
  if not display_df.empty:
    sub_dfs = []
    # Сортируем список машин в алфавитном порядке для предсказуемости
    for vehicle, group_df in display_df.groupby("Машина", sort=True):
      # Внутри мини-таблицы машины сортируем по дате (изолировано)
      sorted_sub_df = group_df.sort_values(by="Дата", ascending=True)
      sub_dfs.append(sorted_sub_df)
